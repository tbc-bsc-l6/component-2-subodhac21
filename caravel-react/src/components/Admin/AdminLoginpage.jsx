...
const AdminLoginpage = () => {
    ...
    const submitForm = (e) =>{
      ...
      if(logindetail.email != "" && logindetail.password != ""){
        ...
        try{
            axios.post("http://127.0.0.1:8000/api/loginAdmin", userData).then((response) => {
                // setLoginDetail({
                //   email: "", 
                //   password: ""
                // });
    ...
    <section className="min-h-screen flex items-stretch text-white ">
        <div className="lg:flex w-1/2 hidden bg-gray-500 bg-no-repeat bg-cover relative items-center bg-[url('https://images.unsplash.com/photo-1577495508048-b635879837f1?ixid=MXwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHw%3D&ixlib=rb-1.2.1&auto=format&fit=crop&w=675&q=80')]" >
            ...
            <div className="w-full px-24 z-10">
                <h1 className="text-5xl font-bold text-left tracking-wide">Admin Page</h1>
                <p className="text-3xl my-4">Login into admin page and control all your Products and orders</p>
            </div>
        ...
        <div className="lg:w-1/2 w-full flex items-center justify-center text-center md:px-16 px-0 z-0" style={{backgroundColor: "#161616"}}>
            ...
            <div className="w-full py-6 z-20">
                <h1 className='text-lg text-[red]'>{load1 === false ? "Invalid Credentials": ""}</h1>
                <h1 className='text-lg text-[red]'>{load1 === "error" ? "Empty Fields": ""}</h1>

                <h1 className="my-6 flex justify-center items-center invert">
                    <img height={60} width={60} src={logo} alt="" />
                </h1>
                ...
                <form onSubmit={(e)=>{submitForm(e)}} className="sm:w-2/3 w-full px-4 lg:px-0 mx-auto">
                    <div className="pb-2 pt-4">
                        <input type="email" onChange={(e)=>changeInput(e)} name="email" id="email" placeholder="Email" className="block w-full p-4 text-lg rounded-sm bg-black" />
                    </div>
                    <div className="pb-2 pt-4">
                        <input onChange={(e)=>changeInput(e)} className="block w-full p-4 text-lg rounded-sm bg-black" type="password" name="password" id="password" placeholder="Password" />
                    </div>
                    {/* <div className="text-right text-gray-400 hover:underline hover:text-gray-100">
                        <a href="#">Forgot your password?</a>
                    </div> */}
                    <div className="px-4 pb-2 pt-4">
                        <button className="uppercase block w-full p-4 text-lg rounded-full bg-indigo-500 hover:bg-indigo-600 focus:outline-none mb-8">sign in</button>
                        <Link className='mt-8 bg-white hover:bg-[#E11D48] duration-75 text-black p-2 rounded-lg' to="/"> Go to Frontend -> </Link>
                    </div>
